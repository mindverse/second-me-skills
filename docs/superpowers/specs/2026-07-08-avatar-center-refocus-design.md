...
| **聊天统一入口（07-13 修订）** | 所有聊天统一走 `/ws-chat/send`：不传 `shareCode` 或传主分身 shareCode 都是和 Personal Agent 聊；传其他指定分身的 shareCode 则和该分身聊。客户端不查询会话接口，也不传递或维护 `sessionId`；请求体只保留 `message` 与可选 `shareCode` |
...
| `POST /api/secondme/ws-chat/send`（统一聊天入口） | 既有 | 不传 `shareCode` 或传主分身 shareCode → Personal Agent；传其他分身 shareCode → 指定分身。请求体仅 `message` / `shareCode`，不接受 `sessionId` 或 `mindId` |
