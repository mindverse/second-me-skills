...
| **聊天双目标（07-09）** | chat 分两个目标：默认和自己的 SecondMe 聊（既有 `/chat/stream`）；给分身链接或 shareCode 时和该分身聊——skill 侧本地 `avatar-sessions.json` 维护 shareCode→sessionId 映射（find-or-create），走 `/ws-chat/send` 同步拿回复。**缺口**：shareCode→mindId 解析未透出，需后端给 `ws-chat/send` 加 `shareCode` 参数（os-main `avatar/chat/init`，微信 bot 已有同款实现），上线前 skill 按 fallback 引导用户走网页聊 |
