...
| `POST /api/secondme/ws-chat/send` | 既有 | 和某人的分身同步聊天（skill 侧维护 session 映射） |
| `POST /api/secondme/ws-chat/send` + `shareCode` 参数 | **新增（小）** | shareCode→mindId 服务端解析，打通「按分身链接聊天」，参照 wechat 模块 `init_avatar_chat` 用法 |
