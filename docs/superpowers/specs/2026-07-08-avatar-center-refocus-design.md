...
| # | 步骤 | 后端支撑 | 依据 / 端点 |
|---|------|----------|-------------|
| 1 | profile 确认 | ✅ 有 | `/api/secondme/user/info` `/profile` `/shades` `/softmemory` |
| 2 | 产品定义讨论（面向谁、提供什么服务） | ✅ 纯访谈 | skill 侧，产出喂给建分身 |
| 3 | 从记忆收集素材 | ✅ 有 | `/api/secondme/memory/key/search` + notes list |
...
1. **签合同**：后端无此能力。方案二选一——(A) 新建「协议展示 + 记录同意」接口；(B) 轻量版：skill 侧展示条款文本 + 本地记录同意，不落后端。
...
| **首轮实测修正（07-09）** | 用户端到端实测反馈修正六项：① 价格单位纠错——`priceUnitAmount = 元 × 100,000`（此前误写「分」），换算表 + 本地校验规则写入 skill（价格 ¥1–¥10,000、配额上限 `ceil(元×10)` 可凑整百、免费试聊 0–10）；② 报价先本地校验再发请求，杜绝试错式失败调用；③ 签约状态显式检查——无查询接口（`avatarMonetizationEnabled` 在 os-main 用户设置，labs 未透出），流程明示「用保存套餐验证签约生效」，透出查询接口列为后端后续项；④ PAID 创建双路径：默认先 FREE 再升级，签约已验证可一步到位；⑤ 访谈选项化——默认建议 + A/B/C，少开放题；⑥ 灵魂注入必做——口头禅/行事作风/方法论采集写入 prompt，是「分身」区别于「普通 agent」的分界线 |
| **聊天双目标（07-09）** | chat 分两个目标：默认和自己的 SecondMe 聊（既有 `/chat/stream`，续聊用 `/chat/session/list` 查后端）；给分身链接或 shareCode 时和该分身聊，走 `/ws-chat/send` 同步拿回复。**session 一律查后端接口，不落本地文件**——`ws-chat/send` 不传 sessionId 时服务端按 userId+mindId find-or-create（os-main `general/message/query_v2`）。**缺口**：shareCode→mindId 解析未透出，需后端给 `ws-chat/send` 加 `shareCode` 参数（os-main `avatar/chat/init`，微信 bot 已有同款实现），上线前 skill 按 fallback 引导用户走网页聊 |
| **签约前置（07-09）** | 签约审核需要时间，改为双入口：阶段 0.5 在用户表达建分身意图时软提示「先提签约申请、与创建并行」（一次性、可跳过）；阶段 5 保留为付费套餐保存前的硬门。目标是让签约申请尽可能早提交 |
| **prompt 方法论（07-09）** | 吸收 fenshen-creator（~/Downloads）实测沉淀：产品定义访谈加「分身类型判断 + 交互姿态六选一（含塌陷模式）」；scenarioPrompt 规范升级为双轨——服务型五段结构（150–500 字）+ 人物型 Layer 结构（1000–20000 字素材自适应），共用七条通用铁律（语言指纹、范例>指令、可指出处、反捏造、收尾减法、说话分寸、反模式写成气味）。姿态塌陷模式与典型问题清单一起构成后续评测种子；fenshen-creator 的 eval/runner 沉淀留作评测模块设计输入 |
...
| `POST /api/secondme/ws-chat/send`（契约改造） | **MR !452 已提** | 按目标路由：不传 `shareCode` → 个人 agent chat（PA mind + agent session）；传 `shareCode` → 指定分身（服务端 `avatar/chat/init` 解析）。请求体仅 message / shareCode / sessionId，mindId 从公开契约移除。分支 `feature/labs-ws-chat-target-routing` → `release/stable` |
