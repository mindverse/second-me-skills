...
2. direct the user to open `https://develop.{DOMAIN}/auth/skills?challenge=<CODE_CHALLENGE>` in their browser (substitute `{DOMAIN}` with the pre-flight `SM_DOMAIN`: `second.me` or `second-me.cn`)
