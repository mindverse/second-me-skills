...
> **视角提示（pre 实测结论）**：用 `ws-chat/send` + 自己分身的 shareCode 自测是可行的——独立会话、**场景人设会生效**，适合验证知识和任务设定。但那是 **owner 视角**：分身认得主人，会直呼其名、自居「你的小己（Second Me）」、可带主人记忆；**访客视角的效果（开场白、对陌生人的称呼与边界）只能用分享链接在网页里以访客身份预览**，两个视角都过一遍才算验收完整。
>
