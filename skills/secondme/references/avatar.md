...
完成语料盘点和信息提取后，才把有依据或经用户确认的候选信息按 Layer 0–3 编译成 `scenarioPrompt`。分身定义是前述过程的产物，不是收集资料前要求用户提供的输入。
...
用户确认后，直接复用本次 `avatarId` 按阶段 6 运行 `smoke`，不重新查询分身或询问模式；用户暂不测试则结束创建流程，并说明之后说“测试我的分身”即可继续。不得未经确认自动创建评测任务。
