...
| 用户名称 | `accessType` | 含义 |
|----------|--------------|------|
| 免费 | `FREE` | 创作者不收费；每条回复消耗访客自己的分身回复额度，不消耗创作者的额度。 |
| 我赞助 | `SPONSORED` | 访客不消耗自己的分身回复额度；每条回复消耗创作者的 💎。 |
| 向我订阅 | `PAID` | 访客订阅后才能聊天，订阅收入归创作者；如配置 `freeChatCount`，可先使用对应的免费试聊次数。 |

面向用户优先使用表中的中文名称，`FREE`、`SPONSORED`、`PAID` 只在 API 语境补充。`FREE` 中的“免费”只表示创作者不收订阅费，不表示聊天不消耗任何额度；不得描述为“访客无限免费聊”“不存在回复额度”或“双方都零消耗”。用户询问当前 `FREE` 分身时，明确说明会消耗访客自己的分身回复额度，不消耗创作者的额度。
