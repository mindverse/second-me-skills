...
| **某人的分身** | 用户给出分身链接（`https://second.me/{route}/avatar/{shareCode}`，`second-me.cn` 域名同样接受）或 shareCode，或点名要和某个分身聊 | 后端按 userId+mindId 自动查找或新建（服务端 find-or-create） | 分身会话聊天（见 [和某人的分身聊天](#和某人的分身聊天)） |
...
1. **解析 shareCode**：分身链接取路径末段（`https://second.me/daihaochen/avatar/1cf5e7728beb` → `1cf5e7728beb`，`second.me` 与 `second-me.cn` 两个域名的链接都有效）；用户直接给 shareCode 就直接用。
