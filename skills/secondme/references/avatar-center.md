...
先判断是否需要问。满足任一条件就**完全跳过本阶段**，不要再提签约申请：

- 本轮对话里已确认签约生效（如保存付费套餐成功过）
- 用户明确说过自己已经完成签约 / 已开通付费能力

注意：**目前没有签约状态查询接口**，不要试图调用不存在的接口去程序化预判——只能依据上面这些会话内信号。都不满足时，在用户表达要创建分身后、进入产品定义之前，**问一次**（整个旅程只问这一次，不反复推销）：
