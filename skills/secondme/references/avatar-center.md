...
0 登录/资料 → 0.5 签约提前申请(可跳过) → 1 产品定义 → 2 收集素材 → 3 建分身
   → 4 定价收费 →〔5 签约·硬门〕→ 6 评测 → 7 交付HTML →〔8 付费〕→ 9 分发
                                                  〔 〕= 付费分身专属
```
...
4. **人设三件套**：身份（我是谁 / 代表谁 / 叫什么）、称呼（怎么称呼访客）、语气（亲切 / 专业 / 简洁…选 1–2 个词）。人物型分身另加：**这个人怎么说话**——口头禅、标志句式、语气词，尽量要真实例句（用户自己的话、聊天记录原文），不接受「幽默风趣」这种形容词。
5. **边界**：哪些问题不回答；什么情况转人工（要到转接话术和联系方式）；素材里没有的内容怎么兜底。
6. **交互姿态**（选一个主姿态，最多主 + 次两个，别贪多）：
...
7. **压力演练**：从典型问题清单抽 1–2 个反问用户：「访客这么问，你希望分身怎么答？」用他的回答校准语气和详略。
