...
```
0 登录/资料/查签约状态 → 0.5 签约提前申请(可跳过) → 1 产品定义 → 2 收集素材 → 3 建分身
...
登录确认后，只要创建意图已明确，立即调用 `GET {BASE}/api/secondme/avatar/contract/status`，并在后台并行执行阶段 1A 的已有信息扫描，再进行任何面向用户的产品内容采集。进入阶段 1 时直接复用扫描结果，不重复查询资料。
...
- `NOT_SIGNED`：在进入产品定义前问一次（整个旅程只问这一次，不反复推销）：
...
5. **灵魂注入**：从现有素材先挖口头禅、句式、行事作风和判断框架；只对确实缺失的部分向用户追问。**判断"缺失"的标准**：句式指纹、口头禅这类内容只能来自用户的真实表达（聊天原文、笔记原句、bio 原话、语音转写），从职业、性格标签**推断**出来的一律算假设、不算证据；真实句式不足 3 条就是缺口，必须追问而不是用推断填满后跳过。追问用低成本方式，一轮 1–2 个：
...
两次查询不能互相替代：用户在产品定义和定价期间，状态可能从 `PENDING` 变为 `SIGNED`。接口失败时不推断状态；FREE 路径可以继续，PAID 路径必须停在保存前。
