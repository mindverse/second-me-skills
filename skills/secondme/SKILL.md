...
```bash
npx skills add https://second.me --skill secondme -y -g
```
...
if [ $((NOW - LAST)) -ge 86400 ]; then
  ...
  if printf '%s\n' "$CHECK_OUTPUT" | grep -qiE "gitlab\.mindverse\.com|mindverse/second-me-skills|github\.com/mindverse/second-me-skills"; then
    npx skills add "${SECONDME_SKILLS_SOURCE:-https://gitlab.mindverse.com/second-me-01/second-me-skills.git}" --skill secondme -y -g >/dev/null 2>&1 || true
  elif printf '%s\n' "$CHECK_OUTPUT" | grep -qiE "second\.me"; then
    npx skills add "${SECONDME_SKILLS_CDN_SOURCE:-https://second.me}" --skill secondme -y -g >/dev/null 2>&1 || true
  fi
  echo "$NOW" > "$STAMP"
fi
